import { z } from "zod";
import { upsertBackend } from "../state/store.js";

const RouteSchema = z.object({
  path: z
    .string()
    .describe("Full API route path (e.g. '/api/customers/{id}')"),
  method: z
...
/**
 * Registers the upsert_backend_schema tool on the MCP server instance.
 * @param {import("@modelcontextprotocol/sdk/server/mcp.js").McpServer} server
 */
export function registerBackendTools(server) {
  server.tool(
      ...
      "Repeated calls with the same controller_id overwrite the previous entry.",
    {
      controller_id: z
        .string()
        .describe(
          "Unique identifier for the .NET controller (e.g. 'CustomerController')"
        ),
      routes: z
        .array(RouteSchema)
        .min(1)
        .describe(
          "Array of API routes/endpoints extracted from the C# controller"
        ),
    },
